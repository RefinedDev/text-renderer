...
use bevy::{
    ...
    gizmos::gizmos::Gizmos, math::Vec2, 
...
impl TextFrame {
    ...
    pub fn update(
        ...
        let center = camera.viewport_to_world_2d(camera_transform,Vec2::new(self.position.x * s_w, self.position.y * s_h)).unwrap();

        let width = s_w * self.size.x * 0.5;
        let height = s_h * self.size.y * 0.5;

        let t_left = Vec2::new(-width, height);
        let t_right = Vec2::new(width, height);
        let b_left = Vec2::new(-width, -height);
        let b_right = Vec2::new(width, -height);

        self.t_left = center + t_left;
        self.t_right = center + t_right;
        self.b_left = center + b_left;
        self.b_right = center + b_right;
    }
